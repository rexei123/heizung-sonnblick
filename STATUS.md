...
## 2bh. Sprint 15d Batterie-Sichtbarkeit — PR2 Frontend (2026-06-05, PR offen)

**Ziel (PR2):** `battery_state` + `battery_low_count` (PR1) im UI sichtbar —
Badge (3+1 an die Achse gekoppelt), Dashboard-Kachel, Batterie-Spalte in der
Geräte-Tabelle, Fehlerstatus-Sortierung über BEIDE Health-Achsen.

**Type-Spiegel (Task 0, §5.63/§5.64 — Pflicht zuerst):**
- `types.ts`: `Device.battery_state` + neuer `BatteryHealthState`,
  `DashboardKpi.battery_low_count`.
- `dashboard.ts`: `dashboardKpiSchema` um `battery_low_count: z.number()` —
  ohne den Key strippt Zod das Feld → Kachel `undefined` (§5.64-Falle).
- `devices.ts` ohne Zod (raw fetch) — nur TS-Typ greift. tsc-Gate grün
  vor Komponenten-Bau.

**Tasks:**
- T1 `BatteryBadge` (`components/patterns/battery-badge.tsx`, Muster von
  `ZoneHealthBadge`): 3+1 Zustände aus `battery_state` (ok grün · warn gelb ·
  kritisch rot · unbekannt grau), prop-getrieben, kein API-Call, defensiver
  Fallback. Material Symbols, Prozent nur im `title`-Tooltip.
- T2 Thermostat-Bubble: `formatPercent` durch `<BatteryBadge>` ersetzt.
- T3 Dashboard: 7. Kachel „Schwache Batterie" (`battery_low_count`,
  warning-soft wenn > 0) + `CARD_META`-Skeleton synchron (sonst 6/7-Drift)
  + Header-Kommentar 6→7.
- T4 `statusScore`-REDESIGN: feste Tabelle über beide Achsen (retired 5 ·
  silent 4 · degraded/suspicious 3 · batt-kritisch 2 · batt-warn 1 · ok 0),
  Maximum pro Gerät; ersetzt die frühere reine `last_seen`-Zeit-Heuristik
  (las `health_state` gar nicht). Beleg-Test: offline (4) schlägt
  batt-kritisch (2).
- T5 Batterie-Spalte in `DevicesTable` (eigener `<td>` zwischen Gerät und
  Zone, §5.66). Detail-„Batterie"-Kachel-Tone von hartkodiertem
  `battery_percent < 20` auf `battery_state` umgestellt (Doppel-Achse
  beseitigt, §5.73) — `warning`-Tone in lokaler `KpiCard` ergänzt.
- T6 Doku: AE-65 Frontend-Teil, CLAUDE.md §5.73, STATUS §2bh + §6.4.

**Tests:** neue `sprint-15d-battery.spec.ts` (Badge-Zustände + statusScore-
Sortier-Beweis). Angepasst: `sprint-14c-dashboard` (Zod-Pflichtfeld + 6→7
zwei Stellen), `devices.spec` (th-Header 4→5, Mocks um `battery_state`),
`sprint-14b-zone-cards` (Bubble Prozent→Badge). Defensiver Badge hält die
übrigen Tabellen-Specs grün ohne Mock-Touch.

**Toolchain (lokal grün, 2026-06-05):**
- `type-check` (tsc): 0 Fehler · `lint` (eslint): 0 · `build` (next): OK
- Playwright E2E: **96 passed** (4 betroffene Specs verifiziert + voller Lauf).

**Backlog-Auflösung:**
- **B-15b-2 ✅ erledigt** in 15d: Stufen-Badge (3+1) ersetzt die Prozent-
...
| B-9.17-4 | **GET-Endpoint-Coverage** (Cutover-Blocker). | ✅ erledigt 2026-05-14 (Sprint 9.17a, §2ao). Code-seitig belegt 2026-06-02 (Sprint 15b-Followup, Backlog-Konsolidierung): alle v1-Router (`dashboard`, `devices`, `global_config`, `heating_zones`, `occupancies`, `overrides`, `room_types`, `rooms`, `rule_configs`, `scenarios`, `users`) haben `require_user`/`require_admin`/`require_mitarbeiter`-Dependencies an jedem Endpoint. Auth-Endpoints (`/auth/login`, `/auth/logout`) bewusst offen; `/auth/me` mit `require_real_user`. Kein ungeschützter v1-Endpoint mehr. |
| B-9.17-5 | Frontend-Wording 429/503 differenzieren. | ✅ erledigt 2026-05-14 (Sprint 9.17a, §2ao T4). |
| B-9.17-6 | `/einstellungen/saison`-Stub mit Sommermodus-Verweis. | ✅ erledigt 2026-05-14 (Sprint 9.17a, §2ao T8). |
| B-9.17-7 | Mojibake Forced-Change-Page-Fehlermeldung. | ✅ erledigt 2026-05-14 (Sprint 9.17a, §2ao T5). |
| B-9.17-8 | Password-Sichtbarkeits-Toggle (Auge-Icon). | ✅ erledigt 2026-05-14 (Sprint 9.17a, §2ao T6). |
| B-9.17-9 | Forced-Change-Page Inline-Fehler pro Feld. | ✅ erledigt 2026-05-14 (Sprint 9.17a, §2ao T7). |
| B-9.17-10 | **`get_current_user`-System-User-Fallback** (Cutover-Blocker). | ✅ erledigt 2026-05-14 (Sprint 9.17a, §2ao T3). Code-seitig belegt 2026-06-02 (Sprint 15b-Followup, Backlog-Konsolidierung): `backend/src/heizung/auth/dependencies.py:76 get_current_user` raised bei `AUTH_ENABLED=true` an jedem Validierungsfehler (fehlender Cookie, ungültiges JWT, fehlender `sub`, ungültige `user_id`-Konversion, User nicht in DB) `_UNAUTHORIZED` (401) — kein stilles Durchlassen. Bei `AUTH_ENABLED=false` zentraler `_system_user(session)`-Pfad mit 503 wenn kein Bootstrap-Admin existiert. |
...
| B-9.17b-4 | **Vicki-002 und Vicki-004 senden seit Pairing keinen Heartbeat** (Sprint-9.17b-Cowork-Beobachtung). | ✅ geschlossen 2026-06-03 (Sprint 15b-Followup, Backlog-Konsolidierung). **Live-Beleg 2026-06-03 04:49Z** (`docker exec deploy-db-1 psql ... sensor_reading + device`): alle 4 Geräte senden frisch (letzte Frames 3-10 min alt), `retired_at IS NULL` bei allen vier. Vicki-004 (`device.id=5`, DevEUI `…e53`) sendet — kein Heartbeat-Ausfall. Marker war veraltet (Beobachtung lag ~3 Wochen zurück, zwischen Sprint 9.17b und Sprint 15b haben Vicki-002/-004 das Heartbeat-Senden aufgenommen). Konsistenz-Check: die Sprint-13b.2-Workflow-4-Retire-Aktion auf id=3 (STATUS §2av Z.2401) wurde **sofort** per `UPDATE device SET retired_at = NULL ... WHERE id = 3` zurückgesetzt (STATUS §2av Z.2418-2428 „Test-State-Drift nach Workflow 4"), daher zeigt der Live-Stand id=3 aktiv — kein Widerspruch zur Doku. |
...
| B-11prep-1 | **Casablanca-FIAS-Anbindung.** | ✅ geschlossen 2026-09-18 (Sprint 17 / AE-71). FIAS entfällt **dauerhaft** — nicht verschoben. Der Mail-Import (AE-66) läuft seit 2026-06-07 produktiv und ist damit der Zielzustand, nicht der Zwischenschritt. Sprint 16a gestrichen. **Folge:** der Import-Watchdog ist jetzt die einzige Absicherung gegen eine ausbleibende Belegungsliste → B-15b-1 wird Go-Live-Blocker mit Frist 25.10.2026. |
...
| B-11prep-4 | **Pilot-Zimmer-Auswahl finalisieren.** | ✅ geschlossen 2026-09-18 (Sprint 17 / AE-70). Gewählt: **54, 102, 207, 310, 406** — 14 Geräte, Montage am 29.09. von unten nach oben (Gateway steht im 3. Stock, der kritischste Funkfall zuerst). 310 liegt im Gateway-Geschoss und ist die Referenz; 406 erhält die Testgeräte 102/103. Gegen `Zimmerliste_seed.csv` geprüft: Etage, Kategorie, Zonenzahl und Summe 14 stimmen. Frühere Nennungen (107, 215) ungültig. |
...
| B-Sprint13a-1 | **`activate_open_window_detection.py` ins neue Sub-Package `heizung.scripts/` migrieren.** | ✅ erledigt 2026-09-17 (Sprint 17 / C3). Skript liegt jetzt unter `backend/src/heizung/scripts/activate_open_window_detection.py`, Aufruf `python -m heizung.scripts.activate_open_window_detection`. Damit ein einheitlicher `-m`-Pfad fuer alle vier Hotelier-Skripte (`seed_rooms`, `pair_devices`, `activate_open_window_detection`, `sync_room_statuses`). Anlass war E3/C3: seit dem Wegfall des OW-Downlinks aus dem Pairing ist dieses Skript der einzige OW-Pfad. Referenzen nachgezogen: RUNBOOK §10e.2, SPRINT-PLAN, `backend/scripts/README.md`, `heizung/scripts/__init__.py`. |
| B-Sprint13a-2 | **`zimmer_nummer` als String statt int.** | ✅ erledigt 2026-09-18 (Sprint 17 / C8). Jetzt `str` mit `max_length=20` passend zu `room.number VARCHAR(20)`. `"101A"` und `"DG"` laufen durch (vorher Absturz), der Zone-Lookup vergleicht direkt ohne `str()`-Konvertierung. |
| B-Sprint13a-3 | **Float-String-Coercion in der Pairing-CSV.** | ✅ erledigt 2026-09-18 (Sprint 17 / C8). `"52.0"` und `"52.00"` werden zu `"52"`. `"52.5"` wird bewusst **nicht** gekürzt — das wäre stilles Datenverbiegen. Drei Tests. |
| B-Sprint13a-4 ✅ | **RUNBOOK-Hinweis zu `app_key` als Cross-Reference-Only dokumentiert.** Erledigt in T7 (Commit `cc27388`). |
| B-Sprint13a-5 | **`pairing_status`-Feld für Downlink-Failure-Recovery.** | ✅ gegenstandslos 2026-09-18 (Sprint 17 / C3). **Doku-Drift korrigiert:** der alte Text behauptete, Migration 0018 führe ein `pairing_status`-Feld ein. Das tut sie nicht — 0018 ist `device_lifecycle` (`retired_at`, `retired_reason`, `replaced_by_device_id` + Partial-Unique-Index). `grep -rn "pairing_status" backend/src backend/alembic` ist **leer**; das Feld hat nie existiert. Der Punkt ist ohnehin erledigt: seit C3 sendet das Pairing keinen Downlink mehr, es gibt also keinen Teil-Zustand, den man markieren müsste. |
...
| B-15b-2 | ✅ **erledigt 2026-06-05 (Sprint 15d PR2, §2bh):** Batterie-`BatteryBadge` (3+1 Zustände aus `battery_state`) ersetzt die Prozentzahl in Bubble + Geräte-Tabelle; exakte Prozente nur noch im Tooltip + Detail-Diagnose. Stufen-Badge statt Prozentzahl (kein Bug, Scheinpräzisions-Hygiene). Codec liefert Geräte-Spannung im 0.1-V-Raster mit 4-Bit-Nibble — Wertebereich 2.0-3.5 V, nibble 15 (= 3.5 V) ist Sättigung am oberen Ende. Effektiv hat das System ~6 unterscheidbare Stufen oberhalb der Wechsel-Schwelle (0/10/30/50/65/80/87/93/100 % an den Codec-Quantisierungs-Stufen 2.7..3.5 V); der Bereich „frisch bis etwas verbraucht" (Live-Beleg 2026-06-02: 3 von 4 Vickis auf nibble=15 saturiert) ist ohne Auflösung. 2-stellige Prozentzahl in der UI ist Scheinpräzision. **Empirie-Beleg 2026-06-02:** ein Gerät kippte zwischen Anchor-Refactor-Snapshot (nibble=15 / 3.5 V) und Live-Verify (nibble=14 / 3.4 V) von 100 % auf 93 % bei minimalem realem Batterie-Abfall — erster konkreter Beleg der Sättigungs-Scheinpräzision in einer einzelnen Codec-Stufe. Vorschlag Folge-Sprint: Stufen-Badge mit ~5 Stufen (`frisch` ≥ 80 %, `gut` ≥ 50 %, `mittel` ≥ 30 %, `warn` ≥ 10 %, `leer` < 10 %), tooltip-Anzeige der Rohspannung für Diagnose. Mit B-15b-1 gemeinsam planbar (UI + Alarm-Anbindung). | 🟢 |

---

### 6.5 — Sprint-17-Folgen (Stand 2026-09-18)

| ID | Inhalt | Priorität |
|---|---|---|
| **B-15b-1** | **Alarm-Versand (Email) für Health- und Import-Alarme.** 🔴 **Go-Live-Blocker, Frist 25.10.2026.** Bis Sprint 16 war das ein Nice-to-have, weil FIAS als PMS-Rückfall galt. Mit AE-71 ist der Mail-Import die einzige PMS-Quelle und der Import-Watchdog ihre einzige Absicherung — und der schreibt heute nur ein `OCCUPANCY_IMPORT_STALE`-Audit und eine Logzeile. **Read-only-Befund 2026-09-18:** es gibt im gesamten Backend keinen Mailversand (`grep -rni "smtplib\|aiosmtp\|send_mail\|EmailMessage\|smtp_host\|sendgrid\|mailgun" backend/src` ist leer, keine Mail-Abhängigkeit in `pyproject.toml`). `emit_health_alert` ist ein Logger-Stub; die STALE-Audit-Zeile hat als einzigen Leser ihren eigenen Idempotenz-Guard — der Log-Endpoint filtert auf `[APPLIED, REJECTED]`. Kein Alarm erreicht den Hotelier aktiv. | 🔴 |
...
| B-17-4 | **Nachrüstliste Montagefenster.** Zonen, die im Schließfenster kein funktionierendes Vicki bekommen haben (Ausfall im Eingangstest) und an denen das Betterspace-Thermostat montiert bleibt. Nachlieferung 3 Stück ~30.10. Die Liste wird in RUNBOOK §10h geführt, nicht hier. | 🟡 |
